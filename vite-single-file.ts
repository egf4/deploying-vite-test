// Borrowed from: https://github.com/richardtallent/vite-plugin-singlefile
	...
	return {
		...
		transformIndexHtml: {
			...
			transform(html: string, ctx?: IndexHtmlTransformContext): IndexHtmlTransformResult {
                ...
                const hasBodyIdx = html.indexOf('</body>');
				// Get the bundle
				let extraCode = ""
				...
				return (hasBodyIdx >= 0 ? html.replace(/<\/body>/, extraCode + "</body>") : (html + extraCode)).replace(reModulepreload, "")
			},
		},
	}
}